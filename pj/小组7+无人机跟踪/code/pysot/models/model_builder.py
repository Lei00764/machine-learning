...
from pysot.models.utile.utile import PRLPP
...
class ModelBuilder(nn.Module):
    ...
    def getcentercuda(self, mapp):
        ...
        x = t.Tensor(
            np.tile(
                (16 * (np.linspace(0, size - 1, size)) + 63)
        ...
        yy = np.int16(
            np.tile(np.linspace(0, size - 1, size).reshape(-1, 1), size).reshape(-1)
        )
    ...
    def forward(self, data):
        ...
        outputs = {}
        outputs["total_loss"] = (
            cfg.TRAIN.LOC_WEIGHT * loc_loss + cfg.TRAIN.CLS_WEIGHT * cls_loss
        )
